...
const FFBManager = require('./ffb_manager');
...
class vJoyDevice extends EventEmitter {
	constructor(id) {
		super();
	 ...
	 * @returns {Boolean}
	 */
	static ffbSupported(id) {
		return vjoy_native.IsDeviceFfb(id);
	}
	 ...
	 * @returns {Object}
	 */
	static getFFBEffects(id) {
		return vjoy_native.getDeviceFfbEffects(id);
	}
	...
	get ffbSupported() {
		return vJoyDevice.ffbSupported(this.id);
	}

	get ffbEffects() {
		return vJoyDevice.getFFBEffects(this.id);
	}
	 ...
	 */
	enableFFBEvents() {
		FFBManager.register(this.id, evt => this.emit("ffb", evt));
	}
